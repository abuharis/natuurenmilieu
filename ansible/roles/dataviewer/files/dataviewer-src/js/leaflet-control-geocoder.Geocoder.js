...
module.exports = {
	"class": L.Control.extend({
		...
		onAdd: function (map) {
			...
			const searchToggle = this._searchToggle = L.DomUtil.create('div', className + '-icon', container);
			searchToggle.id = 'nav-search';
			const searchIcon = L.DomUtil.create('img', '', searchToggle);
			searchIcon.src = './img/nav-search.svg';
			L.DomEvent.addListener(searchToggle, 'click', () => this._toggle());

			const close = L.DomUtil.create('div', 'search-dialog-close', form);
			const closeIcon = L.DomUtil.create('img', '', close);
			closeIcon.src = './img/close-icon.svg';
			L.DomEvent.addListener(close, 'click', () => this._collapse());

			const title = L.DomUtil.create('div', 'search-dialog-title text-blue', form);
			title.innerHTML = `LOCATIE ZOEKEN`;

			const description = L.DomUtil.create('div', 'search-dialog-description', form);
			description.innerHTML = `Zoek op straat of plaatsnaam`;
			

			input = this._input = L.DomUtil.create('input', 'search-input', form);
			input.type = 'text';
			input.id = this._inputId;
			input.placeholder = this.options.placeholder;

			const searchButtonContainer = L.DomUtil.create('div', 'search-button-container', form);
			const searchButton = L.DomUtil.create('button', 'search-dialog-button', searchButtonContainer);
			searchButton.innerHTML = `Zoek`;
			L.DomEvent.addListener(searchButton, 'click', this._doSearch, this);

			const results = this._resultsContainer = L.DomUtil.create('div', 'leaflet-control-geocoder-alternatives-minimized', form);
			// Search results title
			const resultsTitle = L.DomUtil.create('div', 'search-dialog-description', results);
			resultsTitle.innerHTML = 'Zoekresultaten:'

			this._errorElement = L.DomUtil.create('div', className + '-form-no-error', results);
		...
		_geocodeResult: function (results, suggest) {
			if (!suggest && results.length === 1) {
				...
				L.DomUtil.removeClass(this._resultsContainer, 'leaflet-control-geocoder-alternatives-minimized');
		...
		_toggle: function() {
			if (L.DomUtil.hasClass(this._container, 'leaflet-control-geocoder-expanded')) {
                ...
                this._searchToggle.classList.add('icon-selected');
				this._expand();
			}
		},

		_expand: function () {
			navUtils.unselectOtherNavOptions('search');
			L.DomUtil.addClass(this._container, 'leaflet-control-geocoder-expanded');
			L.DomUtil.addClass(this._searchToggle, 'icon-selected');
			this._input.select();
			this.fire('expand');
		},

		_collapse: function () {
			L.DomUtil.removeClass(this._searchToggle, 'icon-selected');
			L.DomUtil.removeClass(this._container, 'leaflet-control-geocoder-expanded');
			L.DomUtil.addClass(this._resultsContainer, 'leaflet-control-geocoder-alternatives-minimized');
